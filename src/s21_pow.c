...
long double s21_pow(double base, double exp) {
    ...
    if ((s21_fabs(base) < S21_ACCUR) && (exp > S21_ACCUR) && (((exp_int && !((int)exp) % 2)) || (!exp_int)))
        return +0;

    // exp is ±0 returns 1 for any base, even when base is NaN
    if (s21_fabs(exp) < S21_ACCUR)
        return 1;

    // base = 1 or base = -1 and exp = +-INF
    if ((base == 1) || ((base == -1) && ((exp == +S21_INFINITY) || (exp == -S21_INFINITY))))
        return 1;

    // base is finite and negative and exp is finite and non-integer
    if (base_is_fin && base < -S21_ACCUR && exp_is_fin && !exp_int && s21_fabs(exp) > 0)
        return S21_NAN;
    ...
    // base is -INF returns +INF if exp is a positive non-integer or positive even integer
        if ((base == -S21_INFINITY) && (exp > S21_ACCUR) && (((exp_int && !((int)exp) % 2)) || (!exp_int)))
            return +S21_INFINITY;

    // base is -INF returns +0 if exp is a negative non-integer or negative even integer
    if ((base == -S21_INFINITY) && (exp < S21_ACCUR) && (((exp_int && !((int)exp) % 2)) || (!exp_int)))
        return +0;
